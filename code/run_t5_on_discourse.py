...
from t5_discourse_embeddings import Embeddings
...
def main():
    ...
    embeddings = Embeddings(path, X_train, checkpoints, 512, f'{task_name}_TRAIN', 'conn')
    metrics = embeddings.calculate()
    embeddings = Embeddings(path, X_test, checkpoints, 512, f'{task_name}_TEST', 'conn')
